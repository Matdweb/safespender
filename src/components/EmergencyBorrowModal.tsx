...
const EmergencyBorrowModal = ({ open, onOpenChange }: EmergencyBorrowModalProps) => {
  ...
  const maxBorrowAmount = 500; // Default max advance amount
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/50 p-3 rounded-lg space-y-2">
            <div className="flex justify-between text-sm font-medium">
              ...
              <span><CurrencyDisplay amount={maxBorrowAmount} className="inline" /></span>
            </div>
            {parseFloat(amount) > 0 && (
              <div className="flex justify-between text-sm">
                <span>Advance Amount:</span>
                <span className="font-medium">
                  <CurrencyDisplay amount={parseFloat(amount) || 0} className="inline" />
                </span>
              </div>
            )}
          </div>
